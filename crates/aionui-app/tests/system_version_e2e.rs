...
fn make_github_release(tag: &str, draft: bool, prerelease: bool, assets: Vec<serde_json::Value>) -> serde_json::Value {
    json!({
        ...
        "body": "Release notes",
        "html_url": format!("https://github.com/finewood2008/centaurai-core/releases/tag/{tag}"),
...
async fn version_check_has_update_with_auth() {
    ...
    Mock::given(method("GET"))
        .and(path("/repos/finewood2008/centaurai-core/releases"))
    ...
    assert_eq!(
        json["data"]["latest"]["html_url"],
        "https://github.com/finewood2008/centaurai-core/releases/tag/v2.0.0"
    );
}
...
async fn full_system_flow_e2e() {
    ...
    assert_eq!(json["data"]["api_key"], "masked:v1:sk-p…1234");
    assert_eq!(json["data"]["api_key_mask"], json["data"]["api_key"]);
    assert_eq!(json["data"]["api_key_present"], true);
    assert!(json["data"]["key_id"].as_str().unwrap().starts_with("key_"));
    assert!(!serde_json::to_string(&json).unwrap().contains("sk-proj-test-key-1234"));
