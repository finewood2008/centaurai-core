//! E2E coverage for the top-level agent-facing `centaurai-core capabilities` index.

use tokio::process::Command;

fn centaurai_core_command() -> Command {
    Command::new(env!("CARGO_BIN_EXE_centaurai-core"))
}

#[tokio::test]
async fn top_level_capabilities_prints_domain_index_without_runtime_env() {
    let output = centaurai_core_command()
        .arg("capabilities")
    ...
    assert_eq!(config["invocation"], "centaurai-core config capabilities");
    ...
    assert_eq!(diagnose["invocation"], "centaurai-core diagnose capabilities");
}
