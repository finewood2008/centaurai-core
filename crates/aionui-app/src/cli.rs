...
pub(crate) struct PrepareManagedResourcesArgs {
    /// Bundle output root. CentaurAI Core writes the managed resources under
...
mod tests {
    ...
    #[test]
    fn command_metadata_uses_centaurai_core_identity() {
        let command = Cli::command();

        assert_eq!(command.get_name(), "centaurai-core");
        assert_eq!(
            command.get_about().map(ToString::to_string),
            Some("CentaurAI Core".to_owned())
        );
    }
