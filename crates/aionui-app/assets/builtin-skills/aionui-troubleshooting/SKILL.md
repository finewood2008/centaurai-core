...
description: >-
  Diagnose a running CentaurAI installation: inspect stuck or errored conversations, read provider health, scheduled task state, MCP server health, team member state, backend health, and CentaurAI Core logs. Use when the user reports CentaurAI is misbehaving, a conversation is stuck, an LLM/provider call is failing, a scheduled task did not run, an MCP server has no tools, a team member is hung, or they ask to troubleshoot CentaurAI.
---
...
4. Treat every command as read-only. To change CentaurAI configuration, use the
...
For a vague "CentaurAI is broken" report, run:
...
"$AIONUI_HELPER_BIN" diagnose logs tail <<'JSON'
{
  "log_dir": "/Users/alex/Library/Logs/CentaurAI",
  "lines": 100,
  "errors_only": true,
  "conversation_id": "conv_123"
}
